...
namespace DataAccessLayer
{
    public class DALForFlightDetails : IDALForFlightDetails
    {
        ...
        public List<FlightDetails> GetFlightDetailsByArrayId(int [] FlightId)
        {
            try
            {
                List<FlightDetails> flightsAvailable=new List<FlightDetails> ();
                int[] ids = FlightId;
                int count = ids.Length;
                for (int i = 0; i < count; i++)
                {
                    var result = dbcontext.FlightDetails.Where(x => x.FlightId == ids[i]).FirstOrDefault();
                    if (result !=null)
                    {
                        flightsAvailable.Add(result);
                    }
                    result = null;
                }
        ...
        public  int [] getFLightIdbySourcedestination(string source, string destination)
        {
            try
            {
                var result = dbcontext.FlightDetails.Where(x => (x.source == source && x.Destination == destination) || (x.Destination==source && x.source==destination)).ToList();
                 var flightIds= result.Select(x => x.FlightId).ToArray();
                return flightIds;
            }
            catch (Exception ex) { throw (ex); }
        }
